# 2.3 Phase-by-Phase Implementation Plan
...
| 3. Specify Requirements | Derive requirements from problem and research | Requirements specification | Section 2.3.3 below |
| 4. Brainstorm/Evaluate/Choose | Compare solution alternatives via decision matrix | Solution selection with rationale | Section 2.3.4 below |
| 5. Develop Prototype | Implement the DPV framework | Working framework in Python | Chapter 3, Sections 3.1--3.2 |
| 6. Test | Run $\tau^2$-bench experiments across 3 scales $\times$ 3 models | Experimental results | Chapter 3, Section 3.3 |
| 7. Communicate | Thesis document + TargetAI recommendations | This thesis + management roadmap | Chapter 3, Sections 3.4--3.5 |

: Mapping of EDP phases to project activities. {#tbl:edp-mapping}

## 2.3.1 Phase 1: Define the Problem
...
| R2 | **Auditability and reversibility.** All changes must be human-readable, versionable, and rollback-able. | Section 1.3.2 (buyer SLA requirements), Section 1.2.2 (alignment tax irreversibility) | Every patch is logged with rationale; the evolved state is serialized to JSON; any prior state can be restored. |
...
## 2.3.4 Phase 4: Brainstorm, Evaluate, and Choose Solution
...
**Manual prompt engineering** satisfies all requirements except R5: it works, but at a cost that scales linearly with deployments (Section 1.3.4), making it the problem rather than the solution.
...
## 2.3.5 Phase 5: Develop and Prototype

The framework was implemented in Python 3.12 using $\tau^2$-bench as the evaluation substrate. The implementation details are presented in Chapter 3, Sections 3.1 (architecture) and 3.2 (implementation and experimental setup).

## 2.3.6 Phase 6: Test

Testing comprised eight experiments across three scales (5, 10, 20 tasks) and three student models (Qwen3 30B-A3B, Qwen3.5 Flash, GLM 4.7 Flash), with all results reported in Chapter 3, Section 3.3. The iterative nature of EDP is visible in the experimental design: results at 5 tasks informed the 10-task experimental design, and the GLM 4.7 Flash failure at 10 tasks led to the decision to drop it from the 20-task evaluation.

## 2.3.7 Phase 7: Communicate

Communication takes three forms: this thesis document, the effectiveness evaluation against project objectives (Section 3.4), and the management roadmap for integration into TargetAI's operations (Section 3.5).
